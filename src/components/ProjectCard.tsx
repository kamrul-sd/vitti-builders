...
export default function ProjectCard({ project, lang, onInquire }: ProjectCardProps) {
  ...
  return (
    <Card className="bg-slate-900 border-slate-800 text-white overflow-hidden flex flex-col group hover:border-slate-700 transition-all duration-300 shadow-md hover:shadow-xl hover:shadow-emerald-950/10 hover:-translate-y-1">
      {/* Project Image & Badge */}
      ...
      <CardHeader className="p-6 pb-0 flex-1">
        {/* Title */}
        <h3 className="text-xl font-bold tracking-tight text-white line-clamp-1 mb-2 font-sans group-hover:text-emerald-400 transition-colors">
          {title}
        </h3>
        ...
        {/* Specs Details */}
        <div className="grid grid-cols-2 gap-y-3.5 gap-x-2 border-t border-slate-800/60 pt-4 text-xs">
          <div className="flex items-center space-x-2">
            <Layout className="h-4 w-4 text-slate-500 shrink-0" />
            <div>
              <span className="block text-[10px] text-slate-500 font-semibold uppercase tracking-wider leading-none mb-1">
                {t.projects.type}
              </span>
              <span className="text-slate-300 font-medium">{typeLabels[project.type]}</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 col-span-2">
            ...
            <div>
              ...
              <span className="text-emerald-400 font-semibold text-sm">{price}</span>
            </div>
          </div>
        </div>
      </CardHeader>
      ...
      <CardFooter className="p-6">
        <Button
          ...
          className="w-full bg-slate-800 hover:bg-emerald-500 text-slate-300 hover:text-white border border-slate-700/60 hover:border-emerald-400 hover:shadow-lg hover:shadow-emerald-500/10 transition-all duration-300"
        >
